        ...
        List {
            Section("Text Models") {
                ForEach(filteredModels.filter { $0.kind == .text }) { model in
                    ModelRowView(model: model, downloader: downloader, onSelectModel: onSelectModel, onRemoveModel: removeModel, onEditModel: { editModelTarget = $0 })
                }
            }
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 12) {
                Button("Select") { onSelectModel(model) }
                    ...
                    .disabled(model.supportsVision ? !model.isVisionReady : !model.isDownloaded)

                if model.ggufURL != nil {
                    Button(model.isDownloaded ? "Update" : "Download") {
